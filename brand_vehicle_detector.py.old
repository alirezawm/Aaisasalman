...
class BrandVehicleDetector:
    ...
    def normalize_text(self, text: str) -> str:
        """نرمال‌سازی متن با پشتیبانی کامل از حروف فارسی"""
        if not text:
            return ""
    ...
    def _normalize_persian_chars(self, text: str) -> str:
        ...
        # Characters to strip (Arabic diacritics and tatweel)
        ARABIC_DIACRITICS = [
            '\u064B', '\u064C', '\u064D', '\u064E', '\u064F', '\u0650', '\u0651', '\u0652', '\u0640'
        ]
        ...
        PY_MAP = {
            ...
            'أ': 'ا', 'إ': 'ا', 'آ': 'ا',  # Different forms of Alef
            'ة': 'ه', 'ۀ': 'ه',  # Teh Marbuta variants
            '\u200c': ' ',  # ZWNJ → space
    ...
    def batch_detect_products(self, product_ids: List[int] = None) -> Dict:
        ...
        try:
            ...
            for product in products:
                # استفاده از نام فارسی اگر موجود باشد، در غیر این صورت نام انگلیسی
                product_name = product.name_fa if product.name_fa else product.name
                detection_result = self.detect_brand_and_vehicle_types(product_name)
    ...
    def get_detection_stats(self) -> Dict:
        ...
        try:
            ...
            products_with_brand = Product.query.filter_by(is_active=True).filter(Product.brand_id.isnot(None)).count()
            products_with_vehicle_types = Product.query.filter_by(is_active=True).join(Product.vehicle_types).count()
            
            # آمار برندها
            total_brands = Brand.query.filter_by(is_active=True).count()
            
            # آمار انواع خودرو
            total_vehicle_types = VehicleType.query.count()
            
            return {
                'total_products': total_products,
                'detected_products': products_with_brand + products_with_vehicle_types,
                ...
                'last_detection_time': 'نامشخص'  # می‌توانید این را از جدول جداگانه‌ای بخوانید
            }
        except Exception as e:
            return {
...
def test_normalization():
    ...
    print("تست نرمال‌سازی حروف فارسی:")
    print("-" * 50)
    
    for test_case in test_cases:
        normalized = detector.normalize_text(test_case)
